"""
Sweep loss mode × epoch sample size for AdaFace.

  2 loss modes:   quality_aware, quality_ignorant
...
the individual has fewer than N). Eval always uses the full gallery.
...
from utils.adaface_reid import count_trainable_parameters, load_best_hyperparams
...
# idx -> (label, loss_mode, epoch_sample_size)
# 2 loss modes × 4 sample sizes = 8 jobs
JOB_CONFIGS = {}
idx = 0
for lm in LOSS_MODES:
    for ss in SAMPLE_SIZES:
        label = f"{lm}_n{ss}"
        JOB_CONFIGS[idx] = (label, lm, ss)
        idx += 1
...
def build_unfiltered_split(dataset, metadata_cache, feasible_individuals, feasibility_config,
                           seed):
    """Unfiltered split: all images (threshold=0.0), quality handled by loss."""
    set_all_seeds(seed)
    ...
    for ind_id in feasible_individuals:
        ...
        # No quality filtering — use all training images
        eligible = filter_training_pool_by_quality(metadata_cache, train_only, 0.0)
        all_train_indices.extend(eligible)
        gallery_info[ind_id] = len(eligible)

        print(f"  {ind_id}: {len(eligible)}/{len(train_only)} train (all, threshold=0.0), {len(val_idx)} val")
...
def run_training(mode_label, train_dataset, val_dataset, individual_to_class, dataset_info,
                 ...
                 loss_mode, epoch_sample_size, epochs, eval_every, seed,
    ...
    adaface_params = count_trainable_parameters(adaface_loss)
    print(f"Trainable: head={head_params:,}, adaface={adaface_params:,}")
    ...
    for epoch in range(epochs):
        ...
        for batch in pbar:
            ...
            optimizer.zero_grad()
            loss = adaface_loss(embeddings, labels, quality)
            loss.backward()
            optimizer.step()
        ...
        if is_eval_epoch:
            query_quality_metrics, open_set_metrics, val_loss = evaluate_recall_with_openset(
                ...
                criterion=adaface_loss, embedding_dim=emb_dim,
...
def main():
    parser = argparse.ArgumentParser(
        description="Sweep loss mode × epoch sample size (8 configs)")
    parser.add_argument("--idx", type=int, required=True,
                        help="Job index 0-7 (2 loss modes × 4 sample sizes)")
    ...
    mode_label, loss_mode, epoch_sample_size = JOB_CONFIGS[idx]
    seed = args.seed
    epochs = args.epochs
    eval_every = args.eval_every
    set_all_seeds(seed)

    output_dir = os.path.join("adaface", "dinov3", "debug")
    ...
    print("=" * 70)
    print(f"AdaFace Tune Epoch Count [{mode_label}]: DINOv3, seed={seed}, idx={idx}")
    print("=" * 70)
    ...
    print(f"\nMode: {mode_label} (loss_mode={loss_mode}, "
    ...
    dataset_info["loss_mode"] = loss_mode
