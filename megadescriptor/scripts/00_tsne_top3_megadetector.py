#!/usr/bin/env python3
"""
Script 00: t-SNE Visualization of MegaDescriptor Embeddings (Top 3 Individuals - MegaDetector Cropped)
Creates an interactive Bokeh plot showing the embedding space of MegaDetector-cropped pelage-visible images
from the top 3 individuals, with shape indicating train/test split and color indicating individual ID.
Uses MegaDetector bounding box cropped images.
"""
...
def get_available_crop_indices(crop_dir):
    """
    Get set of dataset indices for which MegaDetector crops are available
    
    Args:
        crop_dir: Directory containing MegaDetector-cropped images
    
    Returns:
        Set of integers representing available dataset indices
    """
    crop_files = glob(os.path.join(crop_dir, "image_*_crop.jpg"))
    indices = set()
    
    for crop_file in crop_files:
        filename = os.path.basename(crop_file)
        # Extract index from "image_XXXXXX_crop.jpg"
        try:
    ...
    print(f"Found {len(indices)} MegaDetector-cropped images in {crop_dir}")
    return indices


def create_megadescriptor_transform_crop():
    """Create MegaDescriptor-specific transform pipeline for MegaDetector-cropped images"""
    import torchvision.transforms as T
    from PIL import Image
    
    print("MegaDescriptor transform for MegaDetector crops: 384x384 resize + normalize")
...
def create_thumbnail_transform_crop():
    """Create transform for generating thumbnails from MegaDetector-cropped images"""
    import torchvision.transforms as T
    from PIL import Image
...
def filter_dataset_for_crops_and_top3(train_dataset, test_dataset, individual_ids, available_indices):
    """Filter datasets to only include MegaDetector-cropped samples from top 3 individuals with pelage visible"""
    print(f"Filtering datasets for individuals: {', '.join(individual_ids)} (MegaDetector-cropped pelage visible only)")
    print("Note: MegaDetector crops only available for training set")
    
    def is_cropped_target_individual_with_pelage(sample, dataset_idx):
        return (dataset_idx in available_indices and
                sample.get('id', 'Unknown') in individual_ids and 
                sample.get('label', 0) == 1)  # Only pelage visible
    
    # Filter training dataset (only training set has crops)
    filtered_train = []
    for i, sample in enumerate(train_dataset):
        if is_cropped_target_individual_with_pelage(sample, i):
            filtered_train.append((sample, i))
    
    # No test dataset filtering since crops only exist for training set
    filtered_test = []
    
    print(f"Filtered training dataset: {len(filtered_train)} MegaDetector-cropped pelage-visible samples")
    print(f"Test dataset: 0 samples (no MegaDetector crops available for test set)")
    
    return filtered_train, filtered_test


def load_cropped_image(crop_dir, dataset_idx):
    """Load MegaDetector-cropped image for given dataset index"""
    crop_path = os.path.join(crop_dir, f"image_{dataset_idx:06d}_crop.jpg")
    if not os.path.exists(crop_path):
        raise FileNotFoundError(f"MegaDetector crop not found: {crop_path}")
    
    # Load RGB image directly (no alpha channel handling needed)
    rgb_image = Image.open(crop_path).convert('RGB')
    
    return rgb_image


def extract_features_and_thumbnails_crop(filtered_train, filtered_test, crop_dir, model, transform, thumbnail_transform, device, batch_size=16):
    """Extract MegaDescriptor features and generate thumbnails using MegaDetector-cropped images"""
    model.eval()
    features = []
    labels = []
    individual_ids = []
    ...
    print(f"Extracting features from {len(all_samples)} MegaDetector-cropped samples...")
    ...
    for i in range(0, len(all_samples), batch_size):
        ...
        for sample, dataset_idx, split in batch_data:
            try:
                # Load MegaDetector-cropped image
                crop_image = load_cropped_image(crop_dir, dataset_idx)
                
                # Extract features using MegaDescriptor transform
                img_tensor = transform(crop_image)
                batch_images.append(img_tensor)
                
                # Generate thumbnail
                thumbnail_img = thumbnail_transform(crop_image)
                ...
                print(f"Error loading MegaDetector crop for index {dataset_idx}: {e}")
                continue
...
def create_bokeh_plot_crop(tsne_features, individual_ids, pelage_labels, thumbnails, dates, dataset_splits, output_path, feasible_individuals):
    """Create interactive Bokeh plot for MegaDetector-cropped data (training set only)"""
    
    # Create color mapping
    color_map, unique_ids = create_color_palette(individual_ids)
    colors = [color_map[uid] for uid in individual_ids]
    
    # All samples are from training set (since crops only exist for training)
    ...
    p = figure(
        width=1000, 
        height=800,
        title="MegaDescriptor Embedding Space (Top 3 Individuals - MegaDetector Cropped Training Set)",
        tools="pan,wheel_zoom,box_zoom,reset,save"
    )
    
    # Add training samples (all circles)
    p.circle('x', 'y', size=8, color='color', alpha=0.7, 
            source=source, legend_label="Training Set (MegaDetector Cropped)")
    ...
    summary_text = f"""
    <h3>MegaDescriptor Embedding Visualization (Top 3 Individuals - MegaDetector Cropped Training Set)</h3>
    <p><strong>Individuals:</strong> {', '.join(feasible_individuals)}</p>
    <p><strong>Total samples:</strong> {n_total} (training set only)</p>
    <p><strong>All samples:</strong> MegaDetector-cropped pelage visible only (detected climbing poses)</p>
    <p><strong>Note:</strong> Only training set shown - MegaDetector crops not available for test set</p>
    <p><strong>Instructions:</strong> Hover over points to see 256x256 MegaDetector-cropped thumbnails. Colors indicate individuals.</p>
    """
    ...
    print(f"Open in browser to explore {n_total} MegaDetector-cropped training samples from {n_individuals} individuals")


def main():
    parser = argparse.ArgumentParser(description='Create t-SNE visualization of MegaDescriptor embeddings for top 3 individuals (MegaDetector-cropped pelage)')
    ...
    parser.add_argument('--crop_dir', type=str, 
                       default='process_masks/results/detections/cropped_images',
    ...
    print("=" * 80)
    print("MegaDescriptor t-SNE Embedding Visualization (Top 3 Individuals - MegaDetector Cropped)")
    print("=" * 80)
    
    # Set random seed
    set_all_seeds(args.seed)
    
    # Get available MegaDetector crop indices
    available_indices = get_available_crop_indices(args.crop_dir)
    if not available_indices:
        print(f"No MegaDetector crops found in {args.crop_dir}")
        return
    
    # Load top 3 individuals configuration
    ...
    # Filter for MegaDetector-cropped samples from top 3 individuals with pelage visible
    filtered_train, filtered_test = filter_dataset_for_crops_and_top3(
        train_dataset, test_dataset, feasible_individuals, available_indices)
    
    total_samples = len(filtered_train) + len(filtered_test)
    print(f"Total MegaDetector-cropped samples: {total_samples}")
    print(f"  Training: {len(filtered_train)} samples")
    print(f"  Test: {len(filtered_test)} samples")
    
    if total_samples == 0:
        print("No samples found matching criteria (MegaDetector-cropped + top 3 individuals + pelage visible)")
        return
    ...
    print("\nDataset statistics for top 3 individuals (MegaDetector-cropped pelage visible):") 
    ...
    transform = create_megadescriptor_transform_crop()
    thumbnail_transform = create_thumbnail_transform_crop()
    
    print(f"\nUsing device: {device}")
    print(f"Model: MegaDescriptor-L-384")
    print(f"MegaDetector crop directory: {args.crop_dir}")
    
    # Extract features and thumbnails from MegaDetector-cropped images
    start_time = time.time()
    
    (features, labels, individual_ids, pelage_labels, 
     thumbnails, dates, dataset_splits) = extract_features_and_thumbnails_crop(
        filtered_train, filtered_test, args.crop_dir, model, transform, thumbnail_transform, device, batch_size=16
    )
    ...
    if len(features) == 0:
        print("No features extracted - no valid MegaDetector crops found")
        return
    ...
    output_path = os.path.join(args.output_dir, 'tsne_top3_megadetector_cropped.html')
    
    # Create interactive plot
    print(f"\nCreating interactive Bokeh visualization...")
    create_bokeh_plot_crop(tsne_features, individual_ids, pelage_labels, 
                          thumbnails, dates, dataset_splits, output_path, feasible_individuals)
    ...
    print(f"Total MegaDetector-cropped samples: {n_total}")
    print(f"All samples: MegaDetector-cropped pelage visible only")
