...
Uses frozen DINOv3 backbone with raw cosine similarity for open-set recognition:
1. Freezes DINOv3 backbone (only trainable projection head)
2. Uses L2-normalized cosine similarity (no T-Norm)
3. Calibrates threshold using LOO F1 optimization (Positive = Unknown)
4. Measures F1-Score for unknown detection

Key differences from reid_openset_tnorm:
- Score normalization: Raw cosine similarity instead of T-Norm Z-scores
- Threshold scale: Cosine similarity (0.0-1.0) vs Z-scores (0-10)
- Optimization target: F1-Score for unknown detection vs Balanced Accuracy
...
def compute_cosine_similarity(query_emb, gallery_emb):
    ...
    Args:
        query_emb: Query embeddings (n_query, D)
        gallery_emb: Gallery embeddings (n_gallery, D)

    Returns:
        Similarity matrix (n_query, n_gallery) with values in [-1, 1]
    """
...
def compute_open_set_metrics_cosine(
    ...
    Also computes F1-Score where Positive = Unknown (novel individual).
    ...
    Args:
        ...
        score_threshold: Accept threshold (accept if max_score >= threshold)
    ...
    for q_thresh in quality_thresholds:
        # --- Known samples analysis ---
        k_mask = known_query_quality >= q_thresh
        n_known_individuals = 0

        # Track per-individual accept rates and false positives for known
        per_ind_accept = []
        total_fp = 0  # False positives: known samples incorrectly flagged as unknown
        total_known_filtered = 0
            ...
            for ind_label in unique_known_labels:
                ...
                for i in ind_indices:
                    ...
                    if max_score.item() >= score_threshold:
                        # Accepted (not flagged as unknown)
                        if pred_label == ind_label:
                            accepted_correct += 1
                    else:
                        # Flagged as unknown (false positive for unknown detection)
                        # Only count as FP if the prediction would have been correct
                        if pred_label == ind_label:
                            total_fp += 1

                    total_known_filtered += 1
        ...
        total_tp = 0  # True positives: unknown samples correctly flagged
        total_fn = 0  # False negatives: unknown samples incorrectly accepted
        ...
        # Compute F1-Score for unknown detection
        # Positive = Unknown, so:
        # TP = unknowns correctly flagged (rejected)
        # FP = knowns incorrectly flagged as unknown (with correct match)
        # FN = unknowns missed (accepted)
        ...
        # Balanced accuracy (for reference)
...
def evaluate_recall_with_openset(model, train_dataset, val_dataset, individual_to_class,
    ...
    Uses L2-normalized cosine similarity for all scoring, with F1-optimization for
    unknown detection threshold calibration.

    Returns:
        dict: query_quality_metrics with recall at each quality threshold
              open_set metrics with F1-score at each quality threshold
    ...
    # A. Recall@1 (Closed Set) - macro-averaged, using raw cosine scores
    ...
    for thresh in QUERY_QUALITY_THRESHOLDS:
        ...
        # Compute macro-averaged Recall@1 using cosine scores
    ...
    # B. Threshold Calibration (LOO F1 Optimization on Gallery)
    # Calculate Gallery-vs-Gallery cosine similarity
    ...
    # Individual-level LOO Threshold Calibration with F1 Optimization
    #
    ...
    #   4. Grid search for optimal threshold maximizing F1 for this fold
    #
    # Final threshold = mean across all folds

    unique_individuals = gallery_labels.unique().tolist()
    fold_results = []
    thresholds = np.linspace(0.0, 1.0, 101)  # Cosine similarity range
    ...
    for held_out_ind in unique_individuals:
        ...
        n_unknown = len(unknown_max_scores)

        # KNOWN: sample-level LOO within the reduced gallery
        # Group by individual for tracking
        ...
        # Grid search for this fold - optimize F1
        fold_best_f1, fold_best_thresh = 0.0, 0.5
        fold_best_precision, fold_best_recall = 0.0, 0.0

        for thresh in thresholds:
            # True Positives: unknowns correctly flagged (max_score < thresh)
            tp = (unknown_max_scores < thresh).sum().item()

            # False Negatives: unknowns missed (max_score >= thresh)
            fn = n_unknown - tp

            # False Positives: knowns incorrectly flagged as unknown
            # (max_score < thresh AND would have been correct)
            fp = sum(1 for s, c in known_results if s < thresh and c)
            ...
            if f1 > fold_best_f1:
                ...
                fold_best_precision = precision
                fold_best_recall = recall

        fold_results.append({
            'individual': held_out_ind,
            'threshold': fold_best_thresh,
            'f1': fold_best_f1,
            'precision': fold_best_precision,
            'recall': fold_best_recall,
        })

    # Final threshold = mean across folds
    optimal_thresh = np.mean([f['threshold'] for f in fold_results])
    calibration_f1 = np.mean([f['f1'] for f in fold_results])
    calibration_precision = np.mean([f['precision'] for f in fold_results])
    calibration_recall = np.mean([f['recall'] for f in fold_results])
    n_folds = len(fold_results)

    # C. Open Set Metrics (F1-Score for Unknown Detection) - MACRO-AVERAGED
    f1_metrics_by_quality = compute_open_set_metrics_cosine(
    ...
    open_set_metrics = {
        ...
        'by_quality': f1_metrics_by_quality
    }
...
def train_single_config(threshold: float, gallery_size: int, seed: int, args, dataset, config, metadata_cache) -> dict:
    ...
    for epoch in range(EPOCHS):
        ...
        # Track best F1-score for open-set (at q>=0.0 for overall metric)
        by_quality = open_set_metrics.get('by_quality', {})
        f1_q0 = by_quality.get('q>=0.0', {}).get('f1_score', 0.0)
        if f1_q0 > best_f1_score:
            best_f1_score = f1_q0
            best_f1_epoch = epoch + 1

        # Track best harmonic mean of R@1 and F1 (balances closed-set and open-set)
        if (recall_1 + f1_q0) > 0:
            hm = 2 * recall_1 * f1_q0 / (recall_1 + f1_q0)
        else:
            hm = 0.0
        ...
        for q_thresh in QUERY_QUALITY_THRESHOLDS:
            ...
            f1_data = by_quality.get(q_key, {})
            f1 = f1_data.get('f1_score', 0.0)
            prec = f1_data.get('precision', 0.0)
            rec = f1_data.get('recall', 0.0)
            n_k_ind = f1_data.get('n_known_individuals', 0)
            n_u_ind = f1_data.get('n_unknown_individuals', 0)
            print(f"  {q_key}: R@1={r1:.4f} (n={count:3d}), F1={f1:.4f} (P={prec:.2f}, R={rec:.2f})[K={n_k_ind}, U={n_u_ind}]")
        ...
        epoch_history.append({
            ...
            'open_set': open_set_metrics  # Open-set evaluation metrics with F1-score
        })
    ...
    print(f"Best F1 epoch:  {best_f1_epoch}, F1={best_f1_score:.4f}")
...
def main():
    ...
    parser.add_argument('--output_dir', type=str,
                        default='openset_f1_opt/results',
    ...
    print(f"  Threshold Optimization: F1-Score (Positive = Unknown)")
    ...
    print(f"  Open-set evaluation: Enabled (rare individuals, F1 optimization)")
