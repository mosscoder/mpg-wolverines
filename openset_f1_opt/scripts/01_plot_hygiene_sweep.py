#!/usr/bin/env python3
"""
Script 01: Plot Open-Set Hygiene Sweep Results (Raw Cosine + F1 Optimization)

Generates publication-quality figures from open-set evaluation experiment
using DINOv3 backbone with ArcFace loss and raw cosine similarity scoring.

All metrics are computed and stored during training (in epoch_history),
so no GPU inference is needed for plotting.

Figures:
A) Line plot: Recall@1 for known wolverine re-identification - baseline vs optimal
B) Bar chart: Optimal quality thresholds (gallery and query)
C) Line plot: F1-Score for novel individual detection - baseline vs optimal
"""
...
#
# Options:
#   'harmonic_mean'  - 2*R@1*F1 / (R@1+F1) - balances both metrics (DEFAULT)
#   'f1'             - F1-score only (open-set focus)
#   'recall'         - Recall@1 only (closed-set focus)
#   'arithmetic_mean'- (R@1 + F1) / 2
#   'geometric_mean' - sqrt(R@1 * F1)
#
...
def compute_epoch_metric(h: dict, criterion: str, query_thresh: str = "q>=0.0") -> float:
    ...
    Args:
        ...
        criterion: One of 'harmonic_mean', 'f1', 'recall', 'recall_only', 'f1_only'
    ...
    if 'open_set' in h:
        ...
        if query_thresh in by_quality:
            f1 = by_quality[query_thresh].get('f1_score')

    if criterion == 'recall' or criterion == 'recall_only':
        return recall
    elif criterion == 'f1' or criterion == 'f1_only':
        return f1
    elif criterion == 'harmonic_mean':
        if recall is not None and f1 is not None and (recall + f1) > 0:
            return 2 * recall * f1 / (recall + f1)
        return None
        ...
        if recall is not None and f1 is not None and recall > 0 and f1 > 0:
            return np.sqrt(recall * f1)
        return None
    else:
...
def find_best_epoch(all_histories: List[List[dict]],
    ...
    Args:
        ...
        criterion: Optimization criterion - one of:
            'harmonic_mean' (default): 2*R@1*F1 / (R@1+F1) - balances both metrics
            'f1': F1-score only
    ...
    Returns:
        ...
        details_dict contains 'recall', 'f1', and 'criterion_value' at best epoch
    """
        ...
        for history in all_histories:
            for h in history:
                if h['epoch'] == epoch:
                    ...
                    f = compute_epoch_metric(h, 'f1', query_thresh)
                    if r is not None:
        ...
        if metric_values:
            ...
            if mean_metric > best_mean:
                ...
                best_details = {
                    ...
                    'f1': np.mean(f1_values) if f1_values else 0.0,
                    'criterion': criterion,
...
def find_optimal_epoch(all_histories: List[List[dict]],
    ...
    Returns:
        ...
        details_dict contains 'recall', 'f1', 'criterion', and 'criterion_value'
    """
...
def create_combined_figure(results: ResultsCollection, output_dir: str):
    ...
    C) F1-Score for same models/filters as panel A (not independently optimized)
    ...
    for gsize in gallery_sizes:
        ...
        if len(baseline_filtered) > 0:
            ...
            if all_histories and all_histories[0] and 'query_quality_metrics' in all_histories[0][0]:
                ...
                # Collect R@1 and F1 at that epoch with q>=0.0
                r1_values = []
                f1_values = []
                for history in all_histories:
                    for h in history:
                        if h['epoch'] == best_epoch:
                            ...
                            if 'open_set' in h:
                                ...
                                if 'q>=0.0' in by_quality:
                                    f1_values.append(by_quality['q>=0.0']['f1_score'])
                            break
                ...
                if r1_values:
                    ...
                    if f1_values:
                        ...
                        ci_f1 = stats.t.ppf(0.975, len(f1_values) - 1) * stats.sem(f1_values) if len(f1_values) > 1 else 0
                        strategies['baseline']['f1'].append(mean_f1)
                        strategies['baseline']['f1_ci_lower'].append(mean_f1 - ci_f1)
                        strategies['baseline']['f1_ci_upper'].append(mean_f1 + ci_f1)
                ...
                for history in all_histories:
                    for h in history:
                        if h['epoch'] == best_epoch:
                            ...
                            # Also collect F1 at same epoch with same quality filter
                            if 'open_set' in h:
                                by_quality = h['open_set'].get('by_quality', {})
                                if q_thresh in by_quality:
                                    f1_values.append(by_quality[q_thresh]['f1_score'])
                            break
        ...
        if best_r1_values:
            ...
            if best_f1_values:
                mean_f1 = np.mean(best_f1_values)
                ci_f1 = stats.t.ppf(0.975, len(best_f1_values) - 1) * stats.sem(best_f1_values) if len(best_f1_values) > 1 else 0
                strategies['optimal']['f1'].append(mean_f1)
                strategies['optimal']['f1_ci_lower'].append(mean_f1 - ci_f1)
                strategies['optimal']['f1_ci_upper'].append(mean_f1 + ci_f1)
    ...
    # =========================================================================
    # Panel C: F1-Score for novel individual detection
    # =========================================================================
    ...
    for key in ['baseline', 'optimal']:
        s = strategies[key]
        if s['x'] and s['f1']:
            ax3.plot(s['x'], s['f1'], color=colors[key], linewidth=2.5,
                     marker='o', markersize=8, label=s['label'])
            ax3.fill_between(s['x'], s['f1_ci_lower'], s['f1_ci_upper'],
                             color=colors[key], alpha=0.2)

    ax3.set_xlabel('Examples per Individual', fontsize=12)
    ax3.set_ylabel('Novel Individual Detection\n(F1-Score)', fontsize=12)
    ax3.set_xticks(gallery_sizes)

    # Set y-axis range
    all_ci_lower = strategies['baseline']['f1_ci_lower'] + strategies['optimal']['f1_ci_lower']
    all_ci_upper = strategies['baseline']['f1_ci_upper'] + strategies['optimal']['f1_ci_upper']
    if all_ci_lower and all_ci_upper:
        y_min = max(0, np.floor((min(all_ci_lower) - 0.05) / 0.05) * 0.05)
        y_max = min(1, np.ceil((max(all_ci_upper) + 0.05) / 0.05) * 0.05)
...
def plot_cosine_threshold(results: ResultsCollection, output_path: str):
    ...
    ax.set_title('Cosine Decision Threshold by Configuration (F1-Optimized)', fontsize=16)
...
def print_summary_table(results: ResultsCollection):
    ...
    print("\n" + "=" * 100)
    print(f"SUMMARY TABLE: Best Epoch by {DEFAULT_BEST_EPOCH_CRITERION.upper()} (R@1 & F1)")
    ...
    print(f"{'Thresh':<8} {'Gallery':<8} {'Epoch':<6} {'R@1':<8} {'F1':<8} {'H-Mean':<8} {'Seeds':<6}")
    print("-" * 100)
    ...
    for threshold in gallery_thresholds:
        for gallery_size in gallery_sizes:
            ...
            for history in all_histories:
                for h in history:
                    if h['epoch'] == best_epoch:
                        ...
                        f = compute_epoch_metric(h, 'f1', DEFAULT_QUALITY_THRESHOLD)
                        if r is not None:
                            recall_values.append(r)
                        if f is not None:
                            f1_values.append(f)
            ...
            if recall_values and f1_values:
                mean_recall = np.mean(recall_values)
                mean_f1 = np.mean(f1_values)
                # Compute harmonic mean of means
                if (mean_recall + mean_f1) > 0:
                    h_mean = 2 * mean_recall * mean_f1 / (mean_recall + mean_f1)
                else:
                    h_mean = 0.0

                print(f"{threshold:<8.2f} {gallery_size:<8} {best_epoch:<6} "
                      f"{mean_recall:<8.4f} {mean_f1:<8.4f} {h_mean:<8.4f} {len(recall_values):<6}")
    ...
    if metrics:
        ...
        print(f"  R@1={best_data['recall']:.4f}, F1={best_data['f1']:.4f}, H-Mean={best_data['h_mean']:.4f}")
        ...
        best_by_f1 = max(metrics.keys(), key=lambda k: metrics[k]['f1'])

        if best_by_recall != best_key:
            d = metrics[best_by_recall]
            print(f"  Best by R@1: thresh={best_by_recall[0]}, gallery={best_by_recall[1]} "
                  f"-> R@1={d['recall']:.4f}, F1={d['f1']:.4f}")
        if best_by_f1 != best_key:
            d = metrics[best_by_f1]
            print(f"  Best by F1:  thresh={best_by_f1[0]}, gallery={best_by_f1[1]} "
                  f"-> R@1={d['recall']:.4f}, F1={d['f1']:.4f}")


def main():
    parser = argparse.ArgumentParser(description='Plot Open-Set hygiene sweep results (Raw Cosine + F1)')
    parser.add_argument('--results_dir', type=str,
                        default='openset_f1_opt/results',
                        help='Directory containing result JSON files')
    parser.add_argument('--output_dir', type=str,
                        default='openset_f1_opt/figures',
    ...
    print("=" * 60)
    print("Open-Set Evaluation: Gallery Hygiene Sweep Results Analysis (Raw Cosine + F1)")
    print("=" * 60)
    ...
    # Main combined figure (3 panels: R@1, optimal thresholds, F1 for same models)
    ...
    # Cosine threshold plot (replaces T-norm threshold plot)
