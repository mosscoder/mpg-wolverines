#!/usr/bin/env python3
"""
Script 01: Plot DINOv3 + ArcFace Hygiene Sweep Results

Generates publication-quality figures from hygiene filter experiment
using DINOv3 backbone with ArcFace loss.
...
B) Bar chart: Best gallery x query filter combination per examples per individual
C) Gallery x Query quality bar chart: Shows how gallery filtering affects
...
def plot_optimal_thresholds(results: ResultsCollection, output_path: str):
    ...
    ax.set_title('DINOv3 + ArcFace: Optimal Quality Thresholds', fontsize=14)
...
def plot_learning_curves(results: ResultsCollection, output_path: str, gallery_size: int = 64):
    ...
    ax.set_title(f'DINOv3 + ArcFace: Learning Curves (gallery_size={gallery_size})\n'
...
def plot_gallery_vs_query_quality(results: ResultsCollection, output_path: str, gallery_size: int = 64):
    """
    Bar chart: Gallery threshold (hue) x Query quality threshold (x-axis) -> Recall@1.
    ...
    ax.set_title(f'DINOv3 + ArcFace: Gallery x Query Quality (gallery_size={gallery_size})', fontsize=14)
...
def create_main_figure(results: ResultsCollection, output_dir: str):
    ...
    plt.suptitle('DINOv3 + ArcFace: Gallery Hygiene Sweep Results', fontsize=16, y=1.02)
    plt.tight_layout()
...
def print_summary_table(metrics: dict):
    ...
    for (threshold, gallery_size), data in sorted(metrics.items()):
        best_ep = data.get('best_epoch', 100)
    ...
    if metrics:
        ...
        print("-" * 70)
        print(f"Best: threshold={best_key[0]}, gallery_size={best_key[1]}, epoch={best_data.get('best_epoch', 100)} -> "
...
def main():
    parser = argparse.ArgumentParser(description='Plot DINOv3 + ArcFace hygiene sweep results')
    parser.add_argument('--results_dir', type=str,
                        default='reid_dinov3_arcface/results',
                        help='Directory containing result JSON files')
    parser.add_argument('--output_dir', type=str,
                        default='reid_dinov3_arcface/figures',
    ...
    print("=" * 60)
    print("DINOv3 + ArcFace: Gallery Hygiene Sweep Results Analysis")
    print("=" * 60)
    ...
    # Gallery x Query quality bar charts for each gallery size
