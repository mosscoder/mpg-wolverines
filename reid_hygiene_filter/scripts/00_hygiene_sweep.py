#!/usr/bin/env python3
"""
Script 00: Gallery Hygiene Sweep - Filtering Low-Quality Images

Hypothesis: Filtering low-quality images from the Gallery (Database) improves
re-identification performance, even when the Query (Test) set includes all quality levels.

Strategy: Filter training pool by pelage_score >= threshold BEFORE sampling gallery.
No quality weighting during training - just cleaner data.
...
def main():
    parser = argparse.ArgumentParser(description='Gallery hygiene sweep - filter low-quality images')
    ...
    print("=" * 80)
    print(f"Gallery Hygiene Sweep Experiment - Job {args.idx}")
    print("=" * 80)
