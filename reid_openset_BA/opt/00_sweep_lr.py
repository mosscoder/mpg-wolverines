#!/usr/bin/env python3
"""
Script 00: Learning Rate Sweep for DINOv3 Re-ID with Temporal (ymdh) Split

Uses frozen DINOv3 backbone with ArcFace training and temporal (ymdh) train/test split.
Sweeps over 4 learning rates with fixed 224px images.
...
from utils.triplet import ArcFaceLoss, PKBatchSampler
...
def create_dinov3_transform(size=224):
...
def train_single_lr(learning_rate: float, args, dataset, config, metadata_cache) -> dict:
    ...
    transform = create_dinov3_transform(size=224)
...
def main():
    parser = argparse.ArgumentParser(description='LR Sweep for DINOv3 Re-ID with Temporal Split')
    parser.add_argument('--idx', type=int, required=True, help='Job index (0-3 for 4 LRs)')
    parser.add_argument('--overwrite', action='store_true', help='Overwrite existing results')
    parser.add_argument('--output_dir', type=str,
                        default='reid_openset_BA/opt/lr_results',
    ...
    print("=" * 80)
    print(f"LR Sweep - DINOv3 Re-ID with Temporal Split - Job {args.idx}")
    print("=" * 80)
