#!/usr/bin/env python3
"""
Script 01: Image Size Sweep for DINOv3 Re-ID with Temporal (ymdh) Split

Uses frozen DINOv3 backbone with ArcFace training and temporal (ymdh) train/test split.
Sweeps over 4 image sizes with fixed LR=5e-4.
...
def train_single_resize(resize_size: int, args, dataset, config, metadata_cache) -> dict:
    ...
    transform = create_dinov3_transform(size=resize_size)
...
def main():
    parser = argparse.ArgumentParser(description='Image Size Sweep for DINOv3 Re-ID with Temporal Split')
    parser.add_argument('--idx', type=int, required=True, help='Job index (0-3 for 4 sizes)')
    parser.add_argument('--overwrite', action='store_true', help='Overwrite existing results')
    parser.add_argument('--output_dir', type=str,
                        default='reid_openset_BA/opt/resize_results',
    ...
    print("=" * 80)
    print(f"Image Size Sweep - DINOv3 Re-ID with Temporal Split - Job {args.idx}")
    print("=" * 80)
