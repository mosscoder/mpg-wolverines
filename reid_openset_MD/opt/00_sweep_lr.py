#!/usr/bin/env python3
"""
Script 00: Learning Rate Sweep for MegaDescriptor Re-ID with Temporal (ymdh) Split

Uses frozen MegaDescriptor-L-384 backbone with ArcFace training and temporal (ymdh) train/test split.
Sweeps over 10 learning rates with fixed 384px images.
...
IMAGE_SIZE = 384  # MegaDescriptor native resolution
...
def train_single_lr(learning_rate: float, args, dataset, config, metadata_cache) -> dict:
    ...
    transform = create_megadescriptor_transform(size=IMAGE_SIZE)
...
def main():
    parser = argparse.ArgumentParser(description='LR Sweep for MegaDescriptor Re-ID with Temporal Split')
    parser.add_argument('--idx', type=int, required=True, help='Job index (0-9 for 10 LRs)')
    parser.add_argument('--overwrite', action='store_true', help='Overwrite existing results')
    parser.add_argument('--output_dir', type=str,
                        default='reid_openset_MD/opt/lr_results',
    ...
    print("=" * 80)
    print(f"LR Sweep - MegaDescriptor Re-ID with Temporal Split - Job {args.idx}")
    print("=" * 80)
