#!/usr/bin/env python3
"""
Script 00: Open-Set Gallery Hygiene Sweep - C-RADIOv4-H + ArcFace + Raw Cosine Similarity

Uses frozen C-RADIOv4-H backbone with raw cosine similarity for score calibration:
1. Freezes C-RADIOv4-H backbone (only trainable projection head)
...
def create_radio_transform():
    """Create C-RADIOv4-H-specific transform pipeline."""
    # Using 224x224 to match DINOv3 for fair backbone comparison
    return T.Compose([
...
def create_radio_arcface_model(embedding_dim: int = 128, device="cuda"):
    """
    ...
    class RADIOWithHead(nn.Module):
        ...
        def forward(self, x):
            with torch.no_grad():
                # RADIO returns (summary, spatial_features) tuple
                summary, _ = self.backbone(x)
            return self.head(summary)  # Trainable transformation

        def get_trainable_parameters(self):
            return self.head.parameters()

    model = RADIOWithHead(backbone, head)
...
def train_single_config(threshold: float, gallery_size: int, seed: int, args, dataset, config, metadata_cache) -> dict:
    ...
    model, embedding_dim = create_radio_arcface_model(device=device)
    print(f"Using device: {device}")

    # Create transforms and dataset
    transform = create_radio_transform()
...
def main():
    parser = argparse.ArgumentParser(description='Open-Set C-RADIOv4-H + ArcFace + Raw Cosine Gallery Hygiene Sweep')
    parser.add_argument('--idx', type=int, required=True, help='Job index (0-23)')
    parser.add_argument('--overwrite', action='store_true', help='Overwrite existing results')
    parser.add_argument('--output_dir', type=str,
                        default='reid_openset_RADIO/results',
    ...
    print("=" * 80)
    print(f"Open-Set C-RADIOv4-H + ArcFace + Raw Cosine Gallery Hygiene Sweep - Job {args.idx}")
    print("=" * 80)
    ...
    print(f"\nExperiment parameters:")
    print(f"  Model: Frozen C-RADIOv4-H + Projection Head ({EMBEDDING_DIM}-d) + ArcFace")
    ...
    print(f"  Embedding: {EMBEDDING_DIM}-d (trainable projection from C-RADIOv4-H summary)")
