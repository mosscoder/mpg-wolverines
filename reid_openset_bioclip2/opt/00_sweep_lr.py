#!/usr/bin/env python3
"""
Script 00: Learning Rate Sweep for BioCLIP-2 Re-ID with Temporal (ymdh) Split

Uses frozen BioCLIP-2 ViT-L/14 backbone with ArcFace training and temporal (ymdh) train/test split.
Sweeps over 10 learning rates with fixed 224px images.
...
from utils.triplet import ArcFaceLoss, PKBatchSampler, create_bioclip2_arcface_model
...
IMAGE_SIZE = 224  # BioCLIP-2 native resolution
...
def create_bioclip2_transform(size=224):
    """Create BioCLIP-2-specific transform pipeline with CLIP normalization."""
    return T.Compose([
...
def train_single_lr(learning_rate: float, args, dataset, config, metadata_cache) -> dict:
    """Train one configuration and return results."""
    set_all_seeds(SEED)

    # Output path
    filename = f"lr={learning_rate:.6f}.json"
    ...
    transform = create_bioclip2_transform(size=IMAGE_SIZE)
    ...
    result = {
        'config': {
            ...
            'epochs': EPOCHS,
            'seed': SEED,
            'backbone': 'Frozen BioCLIP-2 ViT-L/14',
...
def main():
    parser = argparse.ArgumentParser(description='LR Sweep for BioCLIP-2 Re-ID with Temporal Split')
    parser.add_argument('--idx', type=int, required=True, help='Job index (0-9 for 10 LRs)')
    parser.add_argument('--overwrite', action='store_true', help='Overwrite existing results')
    parser.add_argument('--output_dir', type=str,
                        default='reid_openset_bioclip2/opt/lr_results',
    ...
    print("=" * 80)
    print(f"LR Sweep - BioCLIP-2 Re-ID with Temporal Split - Job {args.idx}")
    print("=" * 80)

    # Check if this job has work to do
    if args.idx >= len(LEARNING_RATES):
        print(f"Job {args.idx} has no work (only {len(LEARNING_RATES)} LRs)")
        return
    ...
    # Get LR for this job
    learning_rate = LEARNING_RATES[args.idx]
    ...
    # Train
    try:
        result = train_single_lr(learning_rate, args, dataset, config, metadata_cache)
