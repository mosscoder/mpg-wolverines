#!/usr/bin/env python3
"""
Script 00: Open-Set Gallery Hygiene Sweep - BioCLIP-2 + ArcFace + Raw Cosine Similarity

Uses frozen BioCLIP-2 ViT-L/14 backbone with raw cosine similarity for score calibration:
1. Freezes BioCLIP-2 backbone (only trainable projection head)
...
def load_best_hyperparams():
    ...
    best_size = 224    # default (BioCLIP-2 native)

    lr_results_dir = 'reid_openset_bioclip2/opt/lr_results'
    ...
    resize_results_dir = 'reid_openset_bioclip2/opt/resize_results'
...
def train_single_config(threshold: float, gallery_size: int, seed: int, args, dataset, config, metadata_cache) -> dict:
    ...
    transform = create_bioclip2_transform()
    ...
    result = {
        ...
        'metadata': {
            ...
            'job_idx': args.idx,
            'transform': f'resize_{BEST_SIZE}_clip_norm',
...
def main():
    parser = argparse.ArgumentParser(description='Open-Set BioCLIP-2 + ArcFace + Raw Cosine Gallery Hygiene Sweep')
    ...
    print("=" * 80)
    print(f"Open-Set BioCLIP-2 + ArcFace + Raw Cosine Gallery Hygiene Sweep - Job {args.idx}")
    print("=" * 80)
    ...
    print(f"\nExperiment parameters:")
    print(f"  Model: Frozen BioCLIP-2 ViT-L/14 + Projection Head ({EMBEDDING_DIM}-d) + ArcFace")
    ...
    print(f"  Embedding: {EMBEDDING_DIM}-d (trainable projection from BioCLIP-2)")
