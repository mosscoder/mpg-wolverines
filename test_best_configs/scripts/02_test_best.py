...
sweeps, then evaluates Recall@1 on the held-out test split. The number of
training epochs is determined by the epoch search (01_best_epoch_search.py),
while quality thresholds come from the hygiene sweep.

Usage:
    python -u test_best_configs/scripts/02_test_best.py --idx $SLURM_ARRAY_TASK_ID --device gpu
"""
...
def load_epoch_search_result(model_name):
    """
    Load the best epoch from the epoch search results (01_best_epoch_search.py).

    Returns: (best_epoch, best_val_r1)
    """
    path = os.path.join('test_best_configs/results/01_best_epoch', f'{model_name}_epoch_search.json')
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Epoch search result not found: {path}\n"
            f"Run 01_best_epoch_search.py first."
        )
    with open(path, 'r') as f:
        data = json.load(f)
    best_epoch = data['results']['best_epoch']
    best_val_r1 = data['results']['best_val_r1']
    print(f"  Epoch search result for {model_name}: best_epoch={best_epoch}, val_r1={best_val_r1:.4f}")
    return best_epoch, best_val_r1
...
def main():
    ...
    # Load epoch from dedicated epoch search (not from hygiene sweep)
    print("\nLoading best epoch from epoch search...")
    best_epoch, epoch_search_val_r1 = load_epoch_search_result(model_name)

    print(f"\nOptimal config for {model_name}:")
    print(f"  LR: {best_lr}, Size: {best_size}, Emb: {best_embedding_dim}")
    print(f"  Epochs: {best_epoch} (from epoch search, val R@1={epoch_search_val_r1:.4f})")
    ...
    result = {
        ...
        'config': {
            ...
            'epoch_source': 'epoch_search',
            'epoch_search_val_r1': epoch_search_val_r1,
            'loss': 'ArcFace',
